...
const WorkflowPath = ({ title, description, steps, color }) => {
  const getColorClass = (colorName) => {
    const colorMap = {
      blue: {
        bg: "bg-blue-50",
      ...
      <div className="p-6">
        <div className="flex flex-col md:flex-row md:items-start">
          {steps.map((step, index) => (
            <div key={index} className="flex flex-1 md:flex-col items-start">
              <div className="flex items-center mb-4 md:mb-0 md:flex-col md:items-center">
                <div
                  className={`flex-shrink-0 w-10 h-10 rounded-full ${colors.light} ${colors.icon} flex items-center justify-center font-bold text-xl`}
                >
                  {index + 1}
                </div>
                ...
                {index < steps.length && (
                  <>
                    <div className="hidden md:block h-12 w-px my-2 border-l-2 border-dashed border-gray-300"></div>
                    <div className="md:hidden w-12 h-px mx-2 border-t-2 border-dashed border-gray-300"></div>
                  </>
                )}
              </div>
